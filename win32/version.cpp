...
#define generation "1"
#define creation "Mon Aug 13 2015 at 15:40:11 EDT"
...
func do_credits(player dbref) {
	for i := 0; infotext[i]; i++ {
		notify(player, infotext[i])
	}
}

func do_version(player dbref) {
	s : fmt.Sprint("Version: %v Compiled on: %v %v", VERSION, creation, debug)
	notify(player, s)
}